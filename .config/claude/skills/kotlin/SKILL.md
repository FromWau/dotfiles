...
- **`references/context-parameters.md`** — choosing between context parameters, receivers, and plain value parameters (the "spotlight principle"): dependency injection/threading, the holder pattern, invisible-context/effect scopes and the bridge pattern, builder/DSL nesting and scope pollution, receiver→context-parameter migration. Read when declaring or reviewing `context(...)` parameters, deciding whether something should be a receiver vs. a context parameter, or designing a DSL/builder or dependency-injection surface.
- **`references/scripting.md`** — running Kotlin as a script (`.kts` / `.main.kts`): inline Maven dependencies via `@file:DependsOn`, why only `.main.kts` resolves them, running from a shell via shebang, always defining a `suspend fun main` entry point (driven by `runBlocking { main(args) }`), CLI arg parsing, and the `sequence { }` can't-`delay` gotcha. Read when writing or running a Kotlin script, adding inline dependencies to a `.kts`, or setting up a shebang-run `.main.kts`.
