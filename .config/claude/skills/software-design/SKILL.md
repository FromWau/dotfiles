...
- **`references/library-design.md`** — designing a library, framework, API, or plugin system that others depend on. Covers extensibility as escape hatches (keep the core small; a good extension point can become your own internal architecture), small composable interfaces vs. fat classes, observability via typed event hooks instead of logging, design-for-testability as a first-class driver (own your scheduling to simulate the clock, inject a fake/faulty filesystem, run I/O in memory), graceful degradation ("play hurt"), free-when-unused immutable structures, and the tail risk of dynamic resource optimization. Illustrated with OkHttp/Okio internals but the moves are language-agnostic. Read when building or reviewing a reusable API, adding an extension point, or making a subsystem testable.
