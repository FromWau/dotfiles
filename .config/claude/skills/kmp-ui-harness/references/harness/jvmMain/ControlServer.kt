...
/**
 * Debug-only localhost control server the agent drives over HTTP. Started once by the desktop
 * [Harness] actual; stripped from release along with the rest of the harness. The routes are a
 * thin transport over the platform-neutral entrypoints in HarnessControl/HarnessSeams; input and
 * screenshots are fully in-process (no external tools). Coordinates are window-content px.
 *
 * - `GET  /layout`     -> JSON: `{ "owners": [<UiNode tree>] }`
 * - `POST /tap`        -> body "x,y"        -> click the element at (x,y); "ok" | "miss"
 * - `POST /longPress`  -> body "x,y"        -> long-press; "ok" | "miss"
 * - `POST /setText`    -> body "x,y,text"   -> set text on the field at (x,y); "ok" | "miss"
 * - `POST /scroll`     -> body "dx,dy" (largest scrollable) or "x,y,dx,dy" (scrollable at x,y);
 *                         +dx scrolls right, +dy scrolls down; "ok" | "miss"
